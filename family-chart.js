...
function oc_text_dimensions (node) {
	...
	var dimensions_Email 	= [ 0, 0 ];
	var dimensions_DOB	= [ 0, 0 ];
	var total_height 	= 0;
	
	node.title_lines = node.subtitle_lines = node.Email_lines = 0;
	...
	// check Email dimensions
	if (node.Email !== undefined) {
		dimensions_Email = oc_text_dimensions_obj (node.Email, oc_style.Email_char_size);
		node.Email_lines = dimensions_Email[2];
		if (node.Email_lines > oc_max_Email_lines)
			oc_max_Email_lines = node.Email_lines;
	}

	// check DOB dimensions
	if (node.DOB !== undefined) {
		dimensions_DOB = oc_text_dimensions_obj (node.DOB, oc_style.title_char_size);
	}
	...
	if (dimensions_Email[0] > oc_max_text_width)
		oc_max_text_width = dimensions_Email[0];
	if (dimensions_DOB[0] > oc_max_text_width)
		oc_max_text_width = dimensions_DOB[0];
		
	// Height increasing with 2 1-- DOB, 1--MARR		
	total_height = dimensions_title[1] + dimensions_subtitle[1] + oc_style.Email_char_size[1] + dimensions_title[1] + dimensions_title[1];
	
	if (total_height > oc_max_text_height)
		oc_max_text_height = total_height ;
	
	// Adding Image Width to global  variables [Height is not required, since height of the photo can accommodate with other heights]
	if(oc_style.Photo_Availability)
		oc_max_text_width = oc_max_text_width  + 5;
...
function oc_draw_obj (node, parent, xoffset, yoffset) {
	...
	// Photo image is drawing 
	if(node.Photo)		
	{
		var PhotoImage = oc_paper.image(node.Photo, x0 + 2, y0 + 2, 50, 50);
		PhotoImage.glow(10, true, 1, 0, 0, "red");
	}
	...
	// draw node title, subtitle & Email
	var box_ypos = 0;
	var title_ypos = y0 + oc_style.inner_padding
					 ...
					 - 2;
	if(oc_style.Photo_Availability)
		var moved_xPos = xc + 50/6; 	// Padding 6 characters to skip photo width 
	var title = oc_paper.text(moved_xPos, title_ypos, node.title);
	title.attr('font-family', 'Courier');
	title.attr('font-size', oc_style.title_font_size);
	title.attr('fill', oc_style.title_color);
	
	// updating box_ypos only.
	box_ypos = title_ypos;
	if (node.subtitle !== undefined) {
		var box_ypos = title_ypos + oc_style.inner_padding
							+ node.subtitle_lines * oc_style.subtitle_char_size[1] / 2;
		var subtitle = oc_paper.text(moved_xPos, box_ypos, node.subtitle);
	...
	if (node.DOB !== undefined) {
		var box_ypos = box_ypos + oc_style.inner_padding
							+ 1 * oc_style.title_char_size[1] / 2;
		var DOB = oc_paper.text(moved_xPos, box_ypos, "DOB:" + node.DOB);
		DOB.attr('font-family', 'Courier');
		DOB.attr('font-size', oc_style.subtitle_font_size);
		DOB.attr('fill', oc_style.title_color);
	}
	if (node.MARR !== undefined) {
		var box_ypos = box_ypos + oc_style.inner_padding
							+ 1 * oc_style.title_char_size[1] / 2;
		var Email = oc_paper.text(moved_xPos, box_ypos, "Married on:" + node.MARR);
		subtitle.attr('font-family', 'Courier');
		subtitle.attr('font-size', oc_style.subtitle_font_size);
		subtitle.attr('fill', oc_style.title_color);
	}
	
	if (node.Email !== undefined) {
		var box_ypos = box_ypos + oc_style.inner_padding
							+ node.Email_lines * oc_style.Email_char_size[1] / 2;
		var Email = oc_paper.text(moved_xPos, box_ypos, node.Email);
		subtitle.attr('font-family', 'Courier');
		subtitle.attr('font-size', oc_style.Email_font_size);
		subtitle.attr('fill', oc_style.Email_color);
	}
